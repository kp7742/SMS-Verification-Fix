.class public Lcom/mega/Mega;
.super Ljava/lang/Object;
.source "Mega.java"


# direct methods
.method public constructor <init>()V
    .locals 0

    .prologue
    .line 11
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method

.method public static Apply(Ljava/lang/String;)Ljava/lang/String;
    .locals 3
    .param p0, "s"    # Ljava/lang/String;

    .prologue
    .line 23
    const-string v0, "com.whatsapp"

    const-string v1, "com.whatsapp"

    invoke-virtual {p0, v0, v1}, Ljava/lang/String;->replace(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)Ljava/lang/String;

    move-result-object v0

    const-string v1, "com.whatsapp.util"

    const-string v2, "com.whatsapp.util"

    invoke-virtual {v0, v1, v2}, Ljava/lang/String;->replace(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)Ljava/lang/String;

    move-result-object v0

    const-string v1, "com.whatsapp.Voip"

    const-string v2, "com.whatsapp.Voip"

    invoke-virtual {v0, v1, v2}, Ljava/lang/String;->replace(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)Ljava/lang/String;

    move-result-object v0

    const-string v1, "com.whatsapp.VideoFrameConverter"

    const-string v2, "com.whatsapp.VideoFrameConverter"

    invoke-virtual {v0, v1, v2}, Ljava/lang/String;->replace(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)Ljava/lang/String;

    move-result-object v0

    const-string v1, "com.whatsapp.util.OpusPlayer"

    const-string v2, "com.whatsapp.util.OpusPlayer"

    invoke-virtual {v0, v1, v2}, Ljava/lang/String;->replace(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)Ljava/lang/String;

    move-result-object v0

    const-string v1, "com.whatsapp.proto"

    const-string v2, "com.whatsapp.proto"

    invoke-virtual {v0, v1, v2}, Ljava/lang/String;->replace(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)Ljava/lang/String;

    move-result-object v0

    const-string v1, "com.whatsapp.util.OpusRecorder"

    const-string v2, "com.whatsapp.util.OpusRecorder"

    invoke-virtual {v0, v1, v2}, Ljava/lang/String;->replace(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method

.method private static HexToByte(Ljava/lang/String;)[B
    .locals 7
    .param p0, "s"    # Ljava/lang/String;

    .prologue
    const/16 v6, 0x10

    .line 259
    invoke-virtual {p0}, Ljava/lang/String;->length()I

    move-result v2

    .line 260
    .local v2, "j":I
    div-int/lit8 v3, v2, 0x2

    new-array v0, v3, [B

    .line 261
    .local v0, "arb":[B
    const/4 v1, 0x0

    .local v1, "i":I
    :goto_0
    if-ge v1, v2, :cond_0

    .line 262
    div-int/lit8 v3, v1, 0x2

    invoke-virtual {p0, v1}, Ljava/lang/String;->charAt(I)C

    move-result v4

    invoke-static {v4, v6}, Ljava/lang/Character;->digit(CI)I

    move-result v4

    shl-int/lit8 v4, v4, 0x4

    add-int/lit8 v5, v1, 0x1

    invoke-virtual {p0, v5}, Ljava/lang/String;->charAt(I)C

    move-result v5

    invoke-static {v5, v6}, Ljava/lang/Character;->digit(CI)I

    move-result v5

    add-int/2addr v4, v5

    int-to-byte v4, v4

    aput-byte v4, v0, v3

    .line 261
    add-int/lit8 v1, v1, 0x2

    goto :goto_0

    .line 264
    :cond_0
    return-object v0
.end method

.method public static a()[B
    .locals 1

    .prologue
    .line 251
    const-string v0, "83566F3B2BAF032A70C59B7685202E25"

    invoke-static {v0}, Lcom/mega/Mega;->HexToByte(Ljava/lang/String;)[B

    move-result-object v0

    return-object v0
.end method

.method public static ab()Ljavax/crypto/SecretKey;
    .locals 5

    .prologue
    .line 246
    const-string v1, "7905796AAFC283ADC6B2AD6CB2137D4F7821F94529D30230D31807613D5B9C28C4A8E25028246B5B17407B6CAFB0378224BF98E06DCF443D87505EE8520886A3"

    invoke-static {v1}, Lcom/mega/Mega;->HexToByte(Ljava/lang/String;)[B

    move-result-object v0

    .line 247
    .local v0, "arrayOfByte":[B
    new-instance v1, Ljavax/crypto/spec/SecretKeySpec;

    const/4 v2, 0x0

    array-length v3, v0

    const-string v4, "PBKDF2WithHmacSHA1And8BIT"

    invoke-direct {v1, v0, v2, v3, v4}, Ljavax/crypto/spec/SecretKeySpec;-><init>([BIILjava/lang/String;)V

    return-object v1
.end method

.method public static b()[B
    .locals 1

    .prologue
    .line 255
    const-string v0, ""

    invoke-static {v0}, Lcom/mega/Mega;->HexToByte(Ljava/lang/String;)[B

    move-result-object v0

    return-object v0
.end method